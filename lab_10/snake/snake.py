...
def save_game_state(username, score, level):
    cur.execute("INSERT INTO snakegame (username, user_score, user_level) VALUES (%s, %s, %s)",
...
def generate_food(walls):
    while True:
        food_x = round(random.randrange(size_of_snake_block,
                                        screen_width - size_of_snake_block - size_of_snake_block) / 10.0) * 10.0
        food_y = round(random.randrange(40, screen_height - size_of_snake_block - size_of_snake_block) / 10.0) * 10.0

        for wall in walls:
            if not any(wall.x <= food_x < wall.x + wall.width and wall.y <= food_y < wall.y + wall.height for wall in
                       walls):
...
def game_loop():
    ...
    food_x = round(
        random.randrange(size_of_snake_block, screen_width - size_of_snake_block - size_of_snake_block) / 10.0) * 10.0
    food_y = round(random.randrange(40, screen_height - size_of_snake_block - size_of_snake_block) / 10.0) * 10.0
    ...
    while not game_over:
        ...
        if x == food_x and y == food_y:
            food_x = round(random.randrange(size_of_snake_block,
                                            screen_width - size_of_snake_block - size_of_snake_block) / 10.0) * 10.0
            food_y = round(
                random.randrange(40, screen_height - size_of_snake_block - size_of_snake_block) / 10.0) * 10.0
            length_of_snake += 1
